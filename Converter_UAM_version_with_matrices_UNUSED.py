...
if __name__ == '__main__':
    ...
    print "test"
    UAM_binary = UAM
    UAM_binary[UAM_binary > 0] = 1
    column_sum = np.sum(UAM_binary, axis=0) # spaltenweise # ist das gleiche wie sum_pc_artist
    row_sum = np.sum(UAM_binary, axis=1)
    filter = (column_sum > 1) & (row_sum > 1) # this filter = what we want to keep
    UAM = UAM[:, filter] # the unpopular artists are now removed
    artists_as_array = artists.keys()
    artists_numpy = np.asarray(artists_as_array)
    artists_numpy = artists_numpy[filter]
    
        # convert artists
    artists = {}
    artists_as_array = artists_numpy.tolist()
    for i in range(0, len(artists_as_array)):
        artists[artists_as_array[i]] = i

    users_as_array = users.keys()
    users_numpy = np.asarray(users_as_array)
    users_numpy = users_numpy[filter]
        # convert users
    users = {}
    users_as_array = users_numpy.tolist()
    for i in range(0, len(users_as_array)):
        users[users_as_array[i]] = i
    ...
    artist_sum_copy[artist_sum_copy == 0] = 1
